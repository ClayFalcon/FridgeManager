...
**Issue #12（設定画面＝在庫ステータス2段階/3段階切替）まで完了し、push済み・CI（Unit Tests / E2E Tests）ともにグリーン。**

- 在庫ステータスの表示モード切替（3段階「買ったばかり/ちょっとある/全くない」↔ 2段階「ある/ない」）
  - 在庫データは常に0/1/2で保存。2段階は表示グルーピングのみ（ちょっとある=あり扱い）。端末ローカル保存
...
Jest 133件・Detox E2E 43件、すべてCIで成功。
...
2. 残Issue: #13（テーマカラーカスタマイズ）、#15（履歴タブ）、#16（プッシュ通知実機テスト）
